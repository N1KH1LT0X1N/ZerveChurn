...
# ── Suppress inherited variable conflicts from parallel upstream branches ────
# `pct` is defined in multiple upstream branches (Validation & Business Alignment
# chain AND Interactive Visualizations chain). Shadow it immediately.
_pct = None       # suppress inherited `pct`
pct  = None       # overwrite with single definition to resolve conflict

print("="*80)
print("COHORT BEHAVIORAL DNA — FEATURE ENGINEERING")
print("="*80)
...
for _uid in all_active_users:
    # Get churn data
    _user_churn = active_users_survival[active_users_survival['user_id'] == _uid].iloc[0]

    # Check behavioral data
    _ubeh = behavioral_fingerprint[behavioral_fingerprint['user_id'] == _uid]
    if len(_ubeh) > 0:
        _ubeh_row = _ubeh.iloc[0]
        _power_score = _ubeh_row['power_user_score']
        _struggle_score = _ubeh_row['struggle_score']
    else:
        _power_score = 0
        _struggle_score = 0
    ...
    if len(_unet) > 0:
        _composite_centrality = _unet.iloc[0]['composite_centrality']
        _is_super_connector = _unet.iloc[0]['is_super_connector']
    else:
        _composite_centrality = 0
        _is_super_connector = False

    unified_profiles.append({
        'user_id': _uid,
        'success_score': _user_churn['success_score'],
        # churn_proxy: low success → high churn risk. Carried so consumers
        # can use a correctly-oriented threshold without re-deriving.
        'churn_proxy': max(0.0, min(100.0, 100.0 - float(_user_churn['success_score']))),
        'success_tier': _user_churn['success_tier'],
        'total_events': _user_churn['total_events'],
        'days_since_last': _user_churn['days_since_last'],
        'is_power_user': _uid in power_users_set,
        'is_network_hub': _uid in network_hubs_set,
        'is_anomalous': _uid in anomalous_users_set,
        'power_user_score': _power_score,
        'struggle_score': _struggle_score,
        'network_centrality': _composite_centrality,
        'is_super_connector': _is_super_connector
    })
...
# Insight 1: Elite users at risk — filter on churn_proxy so we catch the
# elites whose churn risk is high (previous filter on success_score >= 50
# selected the *least* at-risk elites). See docs/repo_state_and_next_steps.md §6.
...
for _ins_i, _insight_item in enumerate(actionable_insights, 1):
    print(f"{_ins_i}. [{_insight_item['priority']}]")
    print(f"   📊 Finding: {_insight_item['insight']}")
    print(f"   💡 Action: {_insight_item['action']}\n")
...
cohort_dna_fig = plt.figure(figsize=(10, 5), facecolor=_bg_c)
_sd_ax = cohort_dna_fig.add_subplot(1, 2, 1)
